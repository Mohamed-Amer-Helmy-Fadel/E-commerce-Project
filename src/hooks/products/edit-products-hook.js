...
import { createProduct, getOneProduct } from '../../reducx/actions/productsAction';
...
import { updateProducts } from './../../reducx/actions/productsAction';
...
const AdminEditProductsHook = (id) => {
    ...
    useEffect(() => {
        if (CatID != 0) {
            const run = async () => {
                await dispatch(getOneCategory(CatID))  //here to get sub subcategory depend in category id so it happening render to to subCat so  useEffect that depend in subCat will run which is down
            }
            run();
        }
    }, [CatID])
